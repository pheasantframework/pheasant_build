...
class PheasantRouterBuilder extends Builder {
  ...
  Future<void> build( BuildStep buildStep) async {
    ...
    var importLine = "import '${buildStep.inputId.pathSegments.last.replaceAll('.dart', '.routes.dart')}'";
    ...
    final paths = matchable.map((e) {
      Iterable<Match> matches = regex.allMatches(e);
      return matches.map((m) => m[1]).join(',');
    }).join(',').split(',');
    if (
      paths.isEmpty 
      || paths.map((e) => e.trim()).where(
        (t) => t.isNotEmpty && t.length >= 3 && t != 'null'
      ).isEmpty
    ) {
      // await buildStep.writeAsString(outputId, "");
      return;
